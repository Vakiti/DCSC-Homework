...
#(['Rto-Adopt', 'Adoption', 'Euthanasia', 'Transfer',
#       'Return to Owner', 'Disposal', 'Died', nan, 'Missing', 'Relocate',
#       'Stolen'], dtype=object)
...
def prep_data(data):
    ...
    data['sex'] = data['Sex upon Outcome'].replace({"Neutered Male":1,
                                                    "Intact Male":1, 
                                                    "Intact Female":0, 
                                                    "Spayed Female":0, 
                                                    "Unknown":np.nan})

    data['is_fixed'] = data['Sex upon Outcome'].replace({"Neutered Male":1,
                                                        "Intact Male":0, 
                                                        "Intact Female":0, 
                                                        "Spayed Female":1, 
                                                        "Unknown":np.nan})
...
# Handle 'Unknown' values in 'Outcome Type' using mode imputation
    mode_outcome_type = data['Outcome Type'].mode().iloc[0]
    data['outcome_type_id'] = data['Outcome Type'].fillna(mode_outcome_type)
    data['outcome_type_id'] = data['Outcome Type'].replace(outcomes_map)
    return data
